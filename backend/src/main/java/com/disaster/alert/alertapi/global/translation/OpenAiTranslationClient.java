...
public class OpenAiTranslationClient {
     ...
     * 로마자라는 중간 표현을 거치게 해 각 단계의 난이도를 낮춘다. "한 음절이라도 한글로
     * 남겨서는 안 된다"는 이 부분 실패를 정확히 겨냥한 문구다.
     *
     * <p><b>예시 지명은 테스트 픽스처와 겹치지 않게 고른다.</b> 프롬프트 예시가 {@code 태안}이고
     * {@code OpenAiTranslationClientRealApiTest}의 고정 원문도 {@code [태안군]}이면, 테스트가
     * 통과해도 "규칙이 임의의 지명에 일반화된다"가 아니라 "예시로 준 지명 하나를 재현한다"만
     * 검증하게 된다. 예시는 순천·경주로 두고 테스트는 태안으로 검증해 일반화를 확인한다.
     *
    ...
    private String buildPrompt(String text, String languageName) {
        return """
                ...
                - 지명·기관명은 %1$s의 통용 표기를 쓴다. 통용 표기가 없으면 먼저 로마자로 옮긴 뒤
                  (예: 순천 → Suncheon, 경주 → Gyeongju) 그 로마자 발음을 %1$s 문자로 표기한다.
