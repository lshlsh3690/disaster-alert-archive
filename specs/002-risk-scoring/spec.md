...
### 사용자 스토리 1 - 알림 클러스터링 결과에 따른 지역 위험도 실시간 갱신 (우선순위: P1)

재난문자가 이벤트로 클러스터링되면(`domain/event`), 위험도 모듈이 해당 이벤트의
유형·강도·정부등급을 합성해 영향 법정동에 위험 점수를 기록하고, 영향받은 시군구의
자기 위험도(source)와 인접 지역으로 번진 최종 위험도(effective)를 즉시 재계산한다.
클러스터링 로직과는 Spring 애플리케이션 이벤트(`AlertClusteredEvent`)로 완전히
분리되어 있어, 클러스터링 서비스는 위험도 계산 방식을 알지 못한다.
...
2. **Given** `AlertClusteredEvent` 수신, **When** `recomputeEventRisk(eventId)`가 실행됨, **Then** `baseScore = weight[유형] × intensity[강도] × severity[정부등급]`가 계산되어 이벤트의 영향 법정동 전체에 `event_region_impact`로 upsert된다 — `backend/.../risk/service/RiskCalculationService.java:100-138`
...
- `primaryDisasterType`이 `"UNKNOWN"`이면 위험도 계산 시 `"기타"`로 치환되어 처리된다(프로파일 조회·확산계수 조회 양쪽 모두) — `backend/.../risk/service/RiskCalculationService.java:100-101`, `backend/.../risk/repository/RegionRiskIndexRepository.java:59-61`
- 강도 추출 정규식이 매칭되지 않거나 해당 유형에 매핑이 없으면 intensity multiplier는 1.0(미반영)으로 처리된다 — `backend/.../risk/service/IntensityExtractor.java:37-41`, `backend/.../risk/repository/IntensityBracketRepository.java:12-22`
- `event.getEmergencyLevel()`이 null이면 severity multiplier는 1.0으로 처리된다(중립) — `backend/.../risk/model/RiskScore.java:26`
- 이벤트 위험도 재계산(`recomputeEventRisk`) 경로는 `region_risk_index`를 직접 갱신하지 않는다 — 자기 프록시 호출로 인한 `@Transactional` 무효화를 피하기 위해 리스너가 별도 프록시 빈 호출로 트리거한다 — `backend/.../risk/service/RiskCalculationService.java:44-51,78-83`
...
- **FR-002**: 시스템은 `baseScore = weight[유형] × intensity[강도] × severity[정부등급]` 공식으로 이벤트 단위 위험 점수를 산출해야 한다(MUST) — `backend/.../risk/service/RiskCalculationService.java:38,122`, `backend/.../risk/model/RiskScore.java:17-19`
- **FR-003**: 유형 가중치(weight)는 `disaster_risk_profile` 테이블에서 조회하며, 없으면 LLM으로 생성해야 한다(MUST) — `backend/.../risk/service/RiskCalculationService.java:104-105`
...
- **FR-006**: 정부 위급단계(`DisasterLevel`)에 따라 severity multiplier를 `LEVEL_3=1.2`, `LEVEL_2=1.0`, `LEVEL_1=0.6`, `null=1.0`으로 적용하고, 이벤트 내 알림 중 최댓값을 사용해야 한다(MUST) — `backend/.../risk/model/RiskScore.java:25-32`, `backend/.../risk/service/RiskCalculationService.java:116-119`
...
- **FR-009**: 시군구 자기 위험도(`source_score`)는 활성 윈도우(30일, `ACTIVE_WINDOW_DAYS`) 내 법정동 impact를 시군구(코드 앞 5자리)로 집계하고 시간 감쇠를 적용한 뒤 합산이 아닌 MAX로 결합해야 한다(MUST) — `backend/.../risk/service/RiskCalculationService.java:156-180`, `backend/.../risk/RiskConstants.java:14`
...
- **FR-013**: 최종 노출 위험도(effective, `risk_score`)는 인접 그래프(`region_adjacency`)를 따라 진원(source)별 BFS로 감쇠 전파(`value × spreadCoeff^hop`)해야 하며, 여러 진원의 영향이 겹치면 MAX로 결합해야 한다(MUST) — `backend/.../risk/service/RiskCalculationService.java:184-237`
