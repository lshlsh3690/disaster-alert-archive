...
import { parseAlertsSearchForm, buildAlertsListParams, buildAlertsStatsParams, type AlertsSearchForm } from "@/lib/alertsSearchParams";
...
function AlertsClientInner() {
  const searchParams = useSearchParams();
  // 서버(page.tsx)가 prefetch할 때 만든 params와 정확히 같은 값이어야 SSR로 미리 받아둔
  // 캐시를 클라이언트가 첫 렌더부터 바로 재사용한다. 예전엔 formState를 {}로 시작하고
  // useEffect에서 searchParams를 반영했는데, 그러면 첫 렌더가 항상 "필터 없음" 상태로
  // 쿼리를 한 번 잘못 날린 뒤 곧바로 실제 필터로 다시 날리는 이중 요청이 있었다.
  ...
  const filteredStatsParams = useMemo(
    () => buildAlertsStatsParams(formState as AlertsSearchForm),
    [formState]
  );
  ...
  useEffect(() => {
    const parsed = parseAlertsSearchForm(searchParams);
