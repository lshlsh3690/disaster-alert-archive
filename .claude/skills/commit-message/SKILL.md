...
## 무엇을 하는가
1. `git status`로 전체 상태를 확인하고, `git diff`(미스테이징)와 `git diff --cached`(이미 스테이징된 추적 파일)를 **둘 다** 실행해 변경 내용을 파악한다 — `git diff`만 보면 이미 스테이징된 내용은 놓친다. `git status`에 `??`로 뜨는 미추적(untracked) 새 파일은 두 diff 어디에도 내용이 안 나오므로, `Read`로 직접 열어보거나 `git add` 후 `git diff --cached`로 확인한다 — 내용을 안 보고 커밋 대상에 포함하지 않는다.
