...
## 무엇을 하는가
1. `git status`/`git diff`로 스테이징 여부와 관계없이 변경 전체를 먼저 파악한다. `git status`에 `??`로 뜨는 미추적(untracked) 새 파일은 `git diff`에 내용이 안 나오므로, `Read`로 직접 열어보거나 `git add` 후 `git diff --cached`로 내용을 확인한다 — 내용을 안 보고 커밋 대상에 포함하지 않는다.
