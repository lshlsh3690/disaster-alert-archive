...
**이 agent는 별도의 권한 상승(`permissionMode` 등)을 갖지 않는다.** `ui-checker`가 확정 버그를 자동으로 이 agent에게 넘기더라도, `Edit`/`Write`/`Bash` 호출 자체는 세션의 기본 권한 모드를 그대로 따른다 — 자동 전달은 "고칠지 말지 사용자에게 먼저 물어보지 않는다"는 뜻이지, 실제 파일 수정·명령 실행의 권한 확인까지 생략한다는 뜻이 아니다.
