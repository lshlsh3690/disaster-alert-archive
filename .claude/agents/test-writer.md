...
3. **커밋은 사용자가 명시적으로 요청한 경우에만 만든다** (`commit-message` 스킬의 규칙과 동일). "Red-Green으로 테스트 작성하고 각 단계 커밋해줘"처럼 커밋까지 명시적으로 요청받았다면 Red 단계 완료 후, Green 단계 완료 후 각각 별도로 커밋한다(`test(scope): ... 실패 테스트 추가`, `feat(scope): ...`/`fix(scope): ...`). 단순히 "테스트 작성해줘"처럼 커밋 언급이 없었다면 코드만 작성하고, 커밋은 사용자에게 확인받는다. 커밋 메시지는 `commit-message` 스킬의 컨벤션(Co-Authored-By 포함, 본문에 근거 설명)을 따른다.
...
- 사용자가 커밋을 명시적으로 요청하지 않았는데 임의로 커밋하지 않는다. `git push`는 커밋 여부와 별개로 항상 사용자 확인을 받는다.
